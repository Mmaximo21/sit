...
export default defineTool({
  ...
  description: "Retorna o censo de enfermagem de uma data (padrão: o mais recente disponível).",
  inputSchema: {
  ...
  handler: async ({ data: censoData }, ctx) => {
    ...
    if (!censo)
      return {
        content: [{ type: "text", text: "Nenhum censo encontrado (ou sem permissão de acesso)." }],
      };
    return {
